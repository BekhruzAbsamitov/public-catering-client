...
function GroceryStores({history}) {
    ...
    async function getActiveStores() {
        return await requestGet(paths.getActiveStores).then(res => {
            setActiveStores(res.data.object)
        })
    }
    ...
    return (
        <div className={'mt-2'}>
            ...
            {isAdmin ?
                <div>
                    <Row>
                        <Col xs={4}><h3>Grocery Store</h3></Col>
                        <Col>
                            <Button size={'sm'} className={'btn btn-success m-1'} onClick={storeToggle}>Add
                                Store</Button>
                        </Col>
                    </Row>
                        ...
                        {activeStores?.map((res, index) =>
                            <tr key={index}>
                                ...
                                <td>
                                    ...
                                    <Link className={'text-white btn btn-sm btn-danger'}
                                          to={`/order/${res.id}`}>Order</Link>
